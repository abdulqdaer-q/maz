...
const Page = () => {
    ...
    const points = (location, nationality, age, sex, experiences, educations, skills) => {
        console.log(location, nationality, age, sex, experiences, skills)
        let totalPoints = 0;
        (location.length > 0 ? totalPoints += 5 : totalPoints += 0);
        (nationality.length > 0 ? totalPoints += 2 : totalPoints += 0);
        (age != null ? totalPoints += 4 : totalPoints += 0);
        (sex != null ? totalPoints += 4 : totalPoints += 0);
                        ...
                        <Card
                            ...
                            actions={[
                                ...
                                <div className="flex  mx-10 gap-5">
                                    <StockOutlined disabled key={"Rate"} /><span >{points(can.userInfo?.residenceCountry, can.userInfo?.nationality, can.userInfo?.birthday, can.userInfo?.gender, can.userInfo?.experiences, can.userInfo?.educations, can.userInfo?.skills)}</span>
                                </div>
                            ]}
                        >
                            <Meta
